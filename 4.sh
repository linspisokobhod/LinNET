...
#!/bin/bash
# ============================================================
#  Lin+ — установка (Node.js, порт 9002)
#  IP: 195.43.142.215
# ============================================================
    ...
    cat > "$INSTALL_DIR/package.json" << 'PKG_EOF'
{
  "name": "linplus",
  "version": "1.0.0",
  "description": "Lin+ — социальная сеть на Node.js",
  "main": "server.js",
  "scripts": {
...
PKG_EOF
    ...
    echo "ℹ️  package.json уже существует — пропускаем"
fi
echo ""
...
echo "🚀 Запуск Lin+ на 195.43.142.215:9002..."
echo "   Откройте: http://195.43.142.215:9002"
...
cat > "$INSTALL_DIR/install-deps.sh" << 'DEPS_EOF'
#!/bin/bash
cd "$(dirname "$0")"
echo "📦 Установка зависимостей (express, express-session, axios)..."
npm install express express-session axios
echo ""
echo "✅ Зависимости установлены. Теперь запустите: ./start.sh"
DEPS_EOF
chmod +x "$INSTALL_DIR/install-deps.sh"

echo "✅ Созданы скрипты: start.sh, stop.sh, restart.sh, install-deps.sh"
echo ""
...
echo "🔗 Redirect:   http://$HOST:$PORT/callback"
echo ""
echo "⚠️  ВАЖНО: зависимости НЕ установлены автоматически!"
echo ""
echo "Следующие шаги:"
echo ""
echo "   1) Установите зависимости:"
echo "      cd $INSTALL_DIR"
echo "      npm install express express-session axios"
echo ""
...
echo "      ./start.sh"
echo ""
echo "   3) Настройте PM2 для работы 24/7:"
echo "      bash ~/linplus/setup-pm2.sh"
echo ""
