...
//  Лайки, комментарии, репосты, удаление постов
// ============================================================
...
function deletePost(postId, userId) {
  ...
  // Удаляем сам пост и все репосты этого поста
  const filtered = posts.filter(x => {
    if (x.id === postId) return false;                // сам пост
    if (x.repost_of === postId) return false;          // прямые репосты
    return true;
  });
...
function renderPost(post, user, opts = {}) {
  ...
  // Кнопка удаления в правом верхнем углу (только для автора)
...
app.post('/post/:id/delete', (req, res) => {
  ...
  // Если удалили со страницы самого поста — идём на главную
...
app.listen(PORT, () => {
  ...
  console.log(`❤️  Лайки, 💬 комментарии, ↗️ репосты, 🗑️ удаление — активны.`);
});
ENDOFFILE
echo "✅ server.js обновлён — добавлено удаление постов"