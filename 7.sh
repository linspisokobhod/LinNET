...
// ================================================================
//  server.js — LinAccounts API с 2FA + cookie-parser
//  Порт: 9001
// ================================================================
...
// ================================================================
//  OAuth 2.0 — для сторонних сайтов
// ================================================================
    ...
    if (sessionToken) {
      db.get('SELECT username FROM sessions WHERE token = ? AND expires > strftime("%s", "now")', [sessionToken], (err, row) => {
        if (row) {
          ...
          const code = uuidv4();
          db.run(
            'INSERT INTO oauth_codes (code, username, client_id, redirect_uri) VALUES (?, ?, ?, ?)',
  ...
  db.get('SELECT * FROM oauth_clients WHERE client_id = ?', [client_id], (err, client) => {
    ...
    db.get('SELECT * FROM oauth_codes WHERE code = ?', [code], (err, codeRow) => {
      ...
      if (codeRow.client_id !== client_id) {
        return res.status(400).json({ error: 'Code does not match client' });
      }
      ...
      const accessExpires = now + 900;      // 15 минут
      const refreshExpires = now + 604800;  // 7 дней

      db.run(
        'INSERT INTO oauth_tokens (token, username, expires) VALUES (?, ?, ?)',
        [accessToken, codeRow.username, accessExpires],
        () => {
          db.run(
            'INSERT INTO refresh_tokens (token, username, expires) VALUES (?, ?, ?)',
            [refreshToken, codeRow.username, refreshExpires],
            () => {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('   POST /api/signup         - регистрация');
